...
Normalumo prielaida netenkinama, taigi rinksimės **Kruskal-Wallis** kriterijų,
kuris galingiausias, kai grupių duomenų skirstiniai vienodos formos:
asimetrija vienodos krypties ir panašaus stiprumo, o sklaida – panašaus
dydžio.
...
conover_rez_df %>%
  ...
  rename("Elektrodas 1 " = group1, "Elektrodas 2 " = group2, "p (koreguotoji)" = p_adj) %>%
  knitr::kable(
    caption = "Lentelė 1.
...
by non-parametric Kruskal-Wallis analysis (significance level α = 0.05) because
the assumption of normality was violated (visually assessed by the QQ chart)
and by a corrective Conover-Iman post-hoc analysis, which showed, that the
changes in amplitudes recorded by the cz electrode were the most different from
the other two electrodes (in terms of p-values), and that the readings of all
the electrodes differed from each other in a statistically significant way,
with the p-value of the p-pair comparisons in all the cases being lower than
